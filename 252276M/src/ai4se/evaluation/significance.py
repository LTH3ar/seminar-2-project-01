...
def exact_mcnemar(y_true, y_pred1, y_pred2):
    res = mcnemar_exact(y_pred1, y_pred2, y_true)
    return {"p_value": res.p_value, "n_discordant": res.n_discordant, "favours_second": res.favours_second}

def holm_bonferroni(p_values, alpha=0.05):
    adjusted = holm(p_values)
    return [{"p_value": raw, "adjusted_p": adj, "significant": adj < alpha} for raw, adj in zip(p_values, adjusted)]
