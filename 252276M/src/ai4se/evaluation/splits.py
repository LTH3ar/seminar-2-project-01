...
from ..repository import InMemoryIssueRepository, IssueRepository, make_repository
...
def stratified_k_fold(issues, n_splits=5, n_folds=None, seed=42):
    k = n_folds if n_folds is not None else n_splits
    if isinstance(issues, (list, tuple)):
        repo = InMemoryIssueRepository(issues)
    else:
        repo = issues
    return list(stratified_folds(repo, n_folds=k, seed=seed))

def train_val_split(texts, labels, val_ratio=0.2, seed=42):
    from collections import defaultdict
    rng = np.random.RandomState(seed)
    label_groups = defaultdict(list)
    for idx, lbl in enumerate(labels):
        label_groups[lbl].append(idx)
    train_idx, val_idx = [], []
    for lbl, indices in sorted(label_groups.items()):
        shuffled = list(indices)
        rng.shuffle(shuffled)
        n_val = max(1, int(len(shuffled) * val_ratio))
        val_idx.extend(shuffled[:n_val])
        train_idx.extend(shuffled[n_val:])
    return [texts[i] for i in train_idx], [labels[i] for i in train_idx], [texts[i] for i in val_idx], [labels[i] for i in val_idx]
