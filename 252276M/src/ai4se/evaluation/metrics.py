...
def macro_f1(*args, **kwargs) -> float:
    """Arithmetic mean of per-class F1 scores.

    Supports either macro_f1(y_true, y_pred) or macro_f1(per_class_metrics_dict).
    """
    if len(args) == 1 and isinstance(args[0], dict):
        pcm = args[0]
        f1s = [m["f1"] for m in pcm.values()]
        return float(np.mean(f1s)) if f1s else 0.0

    if len(args) >= 2:
        y_true, y_pred = args[0], args[1]
        labels = kwargs.get("labels", args[2] if len(args) > 2 else None)
    elif "y_true" in kwargs and "y_pred" in kwargs:
        y_true, y_pred = kwargs["y_true"], kwargs["y_pred"]
        labels = kwargs.get("labels")
    else:
        raise ValueError("macro_f1 expects either (pcm_dict) or (y_true, y_pred)")

    counts = _confusion(y_true, y_pred, labels=labels)
    scores = [_f1(c["tp"], c["fp"], c["fn"]) for c in counts.values()]
...
def confusion_matrix(y_true, y_pred, labels=None):
    ...
    lbls = list(labels or LABELS)
    return _sk_cm(y_true, y_pred, labels=lbls)
...
def per_class_metrics(cm):
    ...
    for i, lbl in enumerate(LABELS):
        ...
        res[lbl] = {"precision": p, "recall": r, "f1": f, "support": int(cm[i, :].sum())}
    return res

def evaluate_predictions(y_true, y_pred, y_prob=None):
    cm = confusion_matrix(y_true, y_pred)
    pcm = per_class_metrics(cm)
    mf1 = float(np.mean([pcm[lbl]["f1"] for lbl in pcm]))
...
def per_repo_and_macro_f1(issues, y_pred):
    repos = sorted(set(iss.repo for iss in issues))
    per_repo = {}
    for r in repos:
        indices = [i for i, iss in enumerate(issues) if iss.repo == r]
        r_true = [issues[i].label for i in indices]
        r_pred = [y_pred[i] for i in indices]
        cm = confusion_matrix(r_true, r_pred)
        pcm = per_class_metrics(cm)
        per_repo[r] = float(np.mean([pcm[lbl]["f1"] for lbl in LABELS]))
    cross_f1 = float(np.mean(list(per_repo.values())))
    return {"cross_repo_f1": cross_f1, "per_repo": per_repo}
