...
application. We implement this with the Repository pattern~\cite{fowler2002peaa}.
...
Stop words are NLTK's English list~\cite{bird2009nltk} plus a domain list of terms appearing in
...
Five scikit-learn~\cite{scikit-learn} pipelines over a shared TF-IDF representation, so that score
...
\textbf{1-D convolutional network}~\cite{kim2014cnn} over learned 100-dimensional embeddings, with
...
\textbf{Frozen encoder baselines.} A pretrained Sentence Transformer~\cite{reimers2019sbert} encodes each
