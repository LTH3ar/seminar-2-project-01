...
\subsection{Error Distribution and Class Confusion}

Across the held-out evaluation split of 1,500 issue reports, 359 instances are misclassified. Table~\ref{tab:error_breakdown} details the distribution across error categories
...
\subsection{Semantic Ambiguity in Class Boundaries}
...
\subsection{Performance Sensitivity to Document Length}
Document length exerts a direct influence on classification accuracy, as shown across corpus quintiles in Table~\ref{tab:length_quintiles_accuracy}
\begin{table}[H]
\centering
...
More striking: \textbf{nine of the twenty-five have a title that explicitly declares a different type than its label}, and in eight of those nine the model agreed with the title rather than the label (Table~\ref{tab:confident_error_examples}).
...
Table~\ref{tab:error_analysis_summary} synthesizes the core findings from the error analysis along with their methodological remediations
\begin{table}[h]
\centering
