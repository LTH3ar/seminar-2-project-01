...
The package is installed in editable mode by the dev container, so no
...
def test_repository_switch_is_one_argument():
    ...
    for implementation in (memory, file_backed):
        ...
        missing = {
            m for m in interface if not callable(getattr(implementation, m, None))
        }
...
def test_public_api_resolves():
    ...
    Added after ``__all__`` drifted from the import block: it advertised
    eighteen names the package did not expose, so ``from ai4se import *``
    raised and ``ai4se.evaluate`` was an AttributeError. Counting the entries
    is not the same as checking they resolve.
    """
    import ai4se

    missing = [name for name in ai4se.__all__ if not hasattr(ai4se, name)]
    assert not missing, f"__all__ advertises names that do not exist: {missing}"


def test_core_package_has_no_heavy_dependencies():
    """``import ai4se`` must work without scikit-learn, torch or transformers.

    The model tracks are optional extras. If one of them were imported at
    package level, a member who installed only ``.[dev]`` could not use the
    data or evaluation layers at all.
    """
    import ast
    from pathlib import Path

    heavy = {"sklearn", "torch", "transformers", "sentence_transformers", "setfit"}
    source = Path(ai4se_init_path()).read_text(encoding="utf-8")
    ...
    assert not (imported & heavy), (
        f"__init__.py imports heavy dependencies: {sorted(imported & heavy)}"
    )

    # The submodules it does import must themselves stay light.
    light_modules = [
        "model",
        "repository",
        "loader",
    ...
    for name in light_modules:
        module_source = (Path(ai4se_init_path()).parent / f"{name}.py").read_text(
            encoding="utf-8"
        )
        for package in heavy:
            assert f"\nimport {package}" not in module_source, (
                f"{name}.py imports {package} at module level"
            )
            assert f"\nfrom {package}" not in module_source, (
                f"{name}.py imports {package} at module level"
            )


def ai4se_init_path() -> str:
    """Absolute path of the package's ``__init__.py``."""
    import ai4se

    return ai4se.__file__
