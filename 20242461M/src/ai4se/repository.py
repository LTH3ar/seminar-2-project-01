...
class IssueRepository(ABC):
    ...
    def __len__(self) -> int:
        """Number of issues held by the repository."""
        return len(self.all())
    ...
    def __getitem__(self, index: int) -> IssueReport:
        """Return the issue at position ``index``."""
        return self.all()[index]

    def __repr__(self) -> str:
        """Show the concrete implementation and how many issues it holds."""
...
class InMemoryIssueRepository(IssueRepository):
    ...
    def __init__(self, issues: Sequence[IssueReport] | None = None) -> None:
        """Create the repository, optionally seeded with ``issues``."""
    ...
    def add(self, issue: IssueReport) -> None:
        """Append a single issue to the internal list."""
...
class FileIssueRepository(IssueRepository):
    ...
    def __init__(
        ...
        """Create the repository and optionally read the backing file."""
        self.path = Path(path)
    ...
    def all(self) -> list[IssueReport]:
        """Return every issue, reading the file first if necessary."""
        self._ensure_loaded()
        return list(self._issues)

    def add(self, issue: IssueReport) -> None:
        """Append an issue in memory; call :meth:`save` to persist it."""
        self._ensure_loaded()
        self._issues.append(issue)

    def clear(self) -> None:
        """Drop the cached content without touching the file on disk."""
        self._issues = []
        self._loaded = True

    def __repr__(self) -> str:
        """Show the backing file name and how many issues it holds."""
...
def make_repository(
    ...
    **kwargs,
    ...
    Args:
        ...
        **kwargs: Forwarded to :class:`FileIssueRepository` (e.g. ``fmt``).
